...
regnum <- optimize_numRegions(inputdata=merge_input ,phenodata=pheno, phenotype="AGE", covariates=NULL,type="numeric",numRegions_set=c(10,20,30,60,80,100,150,200,500,1000))
regnum
```
...
legend("bottomleft", bty="n", legend=c(as.expression(bquote(~ R^2 ~ "= 4.6x10"^-5)),as.expression(bquote(" RMSE = 11.73")))) 
...
legend("bottomleft", bty="n", legend=c(as.expression(bquote(~R^2 ~ "= 8.0x10"^-5)),as.expression(bquote(" RMSE = 11.71")))) 
...
## Create the vignette
library('rmarkdown')
system.time(render('/dcl01/leek/data/sellis/barcoding/phenotype_vignettes/phenopredict-Age.Rmd', 'BiocStyle::html_document'))
