...
if(!file.exists("/dcl01/leek/data/sellis/barcoding/data/merge_input_BMI_nocovars.rda")){
  # source("/dcs01/ajaffe/GTEX/Leek/coverageMatrix/simpleLoad/gtexLoad.R")
    ...
    ncol(cm)
    inputdata<-filter_regions(expression=cm, regiondata=regiondata ,phenodata=pheno, phenotype="BMI", covariates=NULL,type="numeric", numRegions=100)
    assign(paste0("inputdata",'_',chrname),inputdata)
  }
  save(cm_new_chr1, cm_new_chr2, cm_new_chr3, cm_new_chr4, cm_new_chr5, cm_new_chr6, cm_new_chr7, cm_new_chr8, cm_new_chr9, cm_new_chr10, cm_new_chr11, cm_new_chr12, cm_new_chr13, cm_new_chr14, cm_new_chr15, cm_new_chr16, cm_new_chr17, cm_new_chr18, cm_new_chr19, cm_new_chr20, cm_new_chr21, cm_new_chr22, cm_new_chrX, cm_new_chrY, file="data/cm_new_BMI_nocovars.Rda")
  save(cm_regiondata_chr1, cm_regiondata_chr2, cm_regiondata_chr3, cm_regiondata_chr4, cm_regiondata_chr5, cm_regiondata_chr6, cm_regiondata_chr7, cm_regiondata_chr8, cm_regiondata_chr9, cm_regiondata_chr10, cm_regiondata_chr11, cm_regiondata_chr12, cm_regiondata_chr13, cm_regiondata_chr14, cm_regiondata_chr15, cm_regiondata_chr16, cm_regiondata_chr17, cm_regiondata_chr18, cm_regiondata_chr19, cm_regiondata_chr20, cm_regiondata_chr21, cm_regiondata_chr22, cm_regiondata_chrX, cm_regiondata_chrY, file="data/cm_BMI_regiondata_nocovars.Rda")
}
```
...
if(!file.exists("/dcl01/leek/data/sellis/barcoding/data/merge_input_BMI_nocovars.rda")) {
    ...
    save(merge_input, file="/dcl01/leek/data/sellis/barcoding/data/merge_input_BMI_nocovars.rda")
} else {
    load("/dcl01/leek/data/sellis/barcoding/data/merge_input_BMI_nocovars.rda")
}
...
regnum <- optimize_numRegions(inputdata=merge_input ,phenodata=pheno, phenotype="BMI", covariates=NULL,type="numeric",numRegions_set=c(10,20,30,50,100,500,700,900))
regnum
...
if(!file.exists("/dcl01/leek/data/sellis/barcoding/data/predictor_BMI_nocovars.rda")) {
    predictor<-build_predictor(inputdata=merge_input ,phenodata=pheno, phenotype="BMI",      
        covariates=NULL,type="numeric", numRegions=regnum$numRegions)
    save(predictor, file="/dcl01/leek/data/sellis/barcoding/data/predictor_BMI_nocovars.rda")
}else{
    load("/dcl01/leek/data/sellis/barcoding/data/predictor_BMI_nocovars.rda")
}
...
predictions_test <-test_predictor(inputdata=merge_input ,phenodata=pheno, phenotype="BMI", 
...
file.list.cm <- grep("cm_new_BMI_chr",ls(),val=T)
# file.list.inputdata <- grep("inputdata_chr",ls(),val=T)
if(!file.exists("/dcl01/leek/data/sellis/barcoding/data/test_data_BMI_nocovars.rda")) {
    load("data/cm_new_BMI_nocovars.Rda")
    source("/dcl01/leek/data/sellis/barcoding/scripts/gtexLoad.R")
    ...
    for(i in 1:length(chrs)){
        ...
        save(merge_test,file="/dcl01/leek/data/sellis/barcoding/data/test_data_BMI_nocovars.rda")
        # for(i in 1:length(chrs)){
        #   chrname=paste0("chr",chrs[i])
    ...
    load("/dcl01/leek/data/sellis/barcoding/data/test_data_BMI_nocovars.rda")
}
...
predictions<-predict_pheno(inputdata_test=merge_test, phenodata=pheno2,covariates=NULL, phenotype="BMI",type="numeric", predictordata = predictor)
...
## Create the vignette
library('rmarkdown')
system.time(render('/dcl01/leek/data/sellis/barcoding/phenotype_vignettes/phenopredict-BMI.Rmd', output_format = 'BiocStyle::html_document'))

## Extract the R code
library('knitr')
knit('/dcl01/leek/data/sellis/barcoding/phenotype_vignettes/phenopredict-BMI.Rmd', tangle = TRUE)
```
