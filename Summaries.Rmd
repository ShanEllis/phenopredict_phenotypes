...
load("/dcl01/leek/data/sellis/barcoding/plots/colors_44.rda")
```
...
To compare predicted phenotypes with reported phenotypres, confusion matrices had to be calculated for each predicted phenotype: sex, tissue, sample source, and sequencing strategy. Below, predicted phenotypes are along side. Reported phenotypes are along top of tables.

```{r 'confusion_matrix_analysis', echo=FALSE, message = FALSE, warning = FALSE, results="asis"}
...
print(xtable(xtab_tissue), comment=F, type="html")
...
```{r 'sex', message = FALSE, warning = FALSE, echo = FALSE, fig.width = 8, fig.height = 8}
...
```{r 'tissue', message = FALSE, warning = FALSE, echo = FALSE, fig.width = 8, fig.height = 8}

## overall breakdown of predicted tissue
a<-tiss %>% select(predicted) %>% table() %>% melt()
...
a<- tiss %>% mutate(new=(as.character(predicted)==as.character(reported))) %>% group_by(predicted) %>% count(new)
...
## barplot colored by mismatch
  	ggplot(as.data.frame(xtab_tissue), aes(x=predicted, y = Freq, fill=reported)) + 
	geom_bar(stat="identity")+
	labs(title="Tissue")+
...
ggplot(data = as.data.frame(x1), aes(x=predicted, y=reported, fill=Freq)) + 
  geom_tile()+
...
```{r 'sequencingstrategy', message = FALSE, warning = FALSE, echo = FALSE, fig.width = 8, fig.height = 8}
...
```{r 'samplesource', message = FALSE, warning = FALSE, echo = FALSE, fig.width = 8, fig.height = 8}
