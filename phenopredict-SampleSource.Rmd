...
predictions_sra_train_nopheno = as.data.frame(cbind(sra_meta_nopheno$run,"sra",actual=actual,predicted=predicted,perc_correct))
```

# Predict Sample Source in TCGA
```{r 'predict-phenotype-tcga', message = FALSE, warning = FALSE}

regions = predictor$regiondata

if(!file.exists("/dcl01/leek/data/sellis/barcoding/data/regions_CellLine_TCGA.rda")) {

	## save as bedfile for region extraction in TCGA
	#compare to previous sex prediction
	# gr = predictor$regiondata
	# export.bed(gr, con="/dcl01/leek/data/sellis/barcoding/TCGA/TCGA_CellLine_v2.bed")

	## run scripts/run_bwtool_CellLine.sh to extract regions
	setwd('/dcl01/leek/data/sellis/barcoding/SRA/bwtool')
	system('sh /dcl01/leek/data/sellis/barcoding/scripts/run_bwtool_CellLine.sh tcga')
	# data are here: /dcl01/leek/data/sellis/barcoding/TCGA/bwtool_CellLine/coverage_tcga_CellLine

	## check files and merge


	## Parallel environment
	tsv <- dir('/dcl01/leek/data/sellis/barcoding/TCGA/bwtool_CellLine/coverage_tcga_CellLine', pattern = 'tsv', full.names = TRUE)
	names(tsv) <- gsub('.sum.tsv', '', dir('/dcl01/leek/data/sellis/barcoding/TCGA/bwtool/coverage_tcga_CellLine', pattern = 'tsv'))
	...
	mymergeddata = multmerge('/dcl01/leek/data/sellis/barcoding/TCGA/bwtool_CellLine/coverage_tcga_CellLine')
	filenames=list.files(path='/dcl01/leek/data/sellis/barcoding/TCGA/bwtool_CellLine/coverage_tcga_CellLine', full.names=FALSE)

	filenames = gsub('.sum.tsv', '', filenames)
	colnames(mymergeddata)[4:ncol(mymergeddata)] <- filenames

	cov_tcga = mymergeddata
	save(cov_tcga, file="/dcl01/leek/data/sellis/barcoding/data/regions_CellLine_TCGA.rda")
}else{	
	load("/dcl01/leek/data/sellis/barcoding/data/regions_CellLine_TCGA.rda")
}
covmat = cov_tcga
...
#since we know the truth here, let's check and see how we're doing:
  actual = rep("tissue",length(predictions_tcga))
  ...
  #summarize data
	number_match <- sum(predicted==actual)
	perc_correct = sum(predicted==actual)/length(actual)
	number_ssites = nrow(predictor$coefEsts)
...
barplot(as.numeric(mat["percent_correct",])*100, names.arg=c("SRA: training","SRA: test","GTEx","TCGA"),
        main="Sample Source Prediction", ylim=c(0,100), ylab="Accuracy", xlab="Data Set Used", col=colours2, cex.lab=1.3, cex.axis=1.2, cex.main=1.4)
...
## Create the vignette
library('rmarkdown')
system.time(render('/users/sellis/phenopredict/vignettes/phenopredict-SampleSource.Rmd', 'BiocStyle::html_document'))

## Extract the R code
library('knitr')
knit('/users/sellis/phenopredict/vignettes/phenopredict-SampleSource.Rmd', tangle = TRUE)
```
