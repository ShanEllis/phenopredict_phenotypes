...
TCGA_inputdata<-predict_pheno(inputdata_test= TCGA_inputdata, phenodata=pheno, phenotype="SMTS", covariates=NULL,type="factor", predictordata = predictor)
length(predictions)
predicted=predictions


## but, to assess our accuracy, we can only do that where we know the tissue and it's in our predictor set
...
TCGA_inputdata$regiondata <- predictor$regiondata
  ...
  #define predicted
  predicted_sub = predictions

  #summarize data
	number_match <- sum(predicted_sub==actual)
	perc_correct = sum(predicted_sub==actual)/length(actual)
	number_sites = nrow(predictor$coefEsts)
...
#get output for package
actual = md$xml_tumor_tissue_site
...
## make predictions in subset where we have something to compare to...
predictions_sub<-predict_pheno(inputdata_test= SRA_inputdata, phenodata=pheno, phenotype="SMTS", covariates=NULL,type="factor", predictordata = predictor)
# number of samples
length(predictions_sub)

## test for accuracy
actual_all = sra_meta$sharq_beta_tissue
#define predicted
predicted_all = predictions_sub
